Quintus.GameNote = function(Q) {
  ...
  Q.Stage.extend("Stage",{
    ...
    background: function(imageAsset) {
      return this.insert(new Q.Sprite({ asset: imageAsset, type: 0, x: Q.width/2, y: Q.height/2, z: 0 }));
    },
    ...
    points: function() {
      ...
      container.add("tween").set({opacity: 0, x:Q.width/2,  y: -container.p.h/2 }).animate({ opacity: 0.9, y: 70+container.p.h/2 },1,Q.Easing.Quadratic.InOut, {
    ...
    point: function(text,p) {
      this.nextPoint = this.nextPoint || 144
   ...
   scroller: function(asset,titleText,labelText) {
     ...
     var image = this.insert(new Q.Sprite({ asset: asset, x: 450, y: 380, type: 1, opacity: 0, z:1 }).add("tween"));
     var title = this.insert(new Q.UI.Text({ label: titleText.toUpperCase(), size: 50, family: "atrament-web", color: "#C0322D", x: 490, y: 280, align:'left', opacity: 0, z:2, cx: 0, cy: 0  }).add("tween"));
     var label = this.insert(new Q.UI.Text({ label: labelText, size: 24, family: "proxima-nova", weight:400, color: "black", x: 490, y: 340, opacity: 0, z:2, cx:0,cy:0 }).add("tween"));
   ...
   triggerScroller: function() {
     ...
     scrl[0].set({ x: -500, opacity: 0 }).animate({ x: 300, opacity: 1 },1,Q.Easing.Quadratic.Out, { delay: 0.5 });
     scrl[1].set({ x: 1500, opacity: 0 }).animate({ x: 400, opacity: 1 },1,Q.Easing.Quadratic.Out, { delay: 0.5 })
     scrl[2].set({ x: 1500, opacity: 0 }).animate({ x: 400, opacity: 1 },1,Q.Easing.Quadratic.Out, { delay: 0.5 });
   }
  ...
  Q.UI.Container.extend("PointContainer",{
    init: function() {
      this._super({
        w: "75%", h: 600, radius: 20, opacity:1, shadow: true, type:0, fill: "#FFF"
      });
    }
  });
  ...
  Q.transitionSlide = function(from,to,skipDisable) {
    ...
    if(to < 1) {
      Q(".platformerControls",Q.activeSlideStage).set({ x: 1 });
      return;
    } else if(!Q.scene("slide" + to)) {
      Q(".platformerControls",Q.activeSlideStage).set({ x: 1023 });
      return;
    }
    ...
    if(from < to) {
      ...
      if(!Q(".platformerControls",Q.activeSlideStage).first().p.skipMove) {
        Q(".platformerControls",Q.activeSlideStage).set({ x: 1024 - 100 });
      }
    }
  };
  ...
  Q.Sprite.extend("Presenter",{
    ...
    step: function(dt) {
      ...
      if(this.stage.pointsList && this.stage.isVisible) { 
        var len = this.stage.pointsList.length;
        for(var i = 0;i < len;i++) {
          var point = this.stage.pointsList[i];

          if((this.p.x-50.0) / (1024-100) > (i / len) ) {
            if(!this.triggered[i]) {
              this.stage.trigger("point",i);
              this.triggered[i] = true;
            }
          } else if(!this.stage.pointsList[i].hidden) {
            this.stage.trigger("hidePoint",i);
